...
st.set_page_config(page_title="23-Saeulen-Modell v4.1", page_icon="📊", layout="wide")
...
st.title("📊 23-Saeulen-Modell v4.1")
...
if price > ma20 > ma50 > ma200:
    s3, s3a, s3t = 100, "🟢", "Bull Stack"
elif price < ma20 < ma50 < ma200:
    s3, s3a, s3t = 10, "🔴", "Bear Stack"
else:
    s3, s3a, s3t = 50, "🟡", "Gemischt"
...
if ret5 > 0 and vol5 > vol20 * 1.15:
    s5, s5a, s5t = 100, "🟢", f"Vol {vol5/vol20:.1f}x | OBV {obv_trend}"
elif ret5 > 0:
    s5, s5a, s5t = 65, "🟡", f"Vol normal | OBV {obv_trend}"
elif vol5 > vol20 * 1.15:
    s5, s5a, s5t = 20, "🔴", "Vol hoch bei Kursrueckgang"
else:
    s5, s5a, s5t = 40, "🟡", "Durchschnittliches Volumen"

if atr_pct < 3:
    s6, s6a, s6t = 100, "🟢", f"ATR {atr_pct:.1f}% niedrig"
elif atr_pct < 7:
    s6, s6a, s6t = 65, "🟡", f"ATR {atr_pct:.1f}% normal"
else:
...
with t2:
    st.dataframe(pd.DataFrame({
        ...
        "Status": ["🟢","🟢",sg_earn,reg_amp,"🟢" if hd >= 30 else "⚫","🟢" if kb >= 3 else ("🟡" if kb == 2 else "🔴")],
