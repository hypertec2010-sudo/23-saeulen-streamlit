...
# cookie_secret = "EIN_LANGER_ZUFAELLIGER_WERT"
...
def require_google_login():
    """
    Streamlit Google OIDC login gate.
...
def render_user_bar():
    try:
        ...
        with c1:
            user_name = getattr(st.user, "name", None) or st.user.get("email", "angemeldet")
            user_email = st.user.get("email", "")
            if user_email:
                st.caption(f"Angemeldet als: {user_name} ({user_email})")
            else:
                st.caption(f"Angemeldet als: {user_name}")
        with c2:
            st.button("Abmelden", on_click=st.logout, key="google_logout_main")
    except Exception:
        pass
...
def _ultra_short_term_signal_bg(signal_label):
    label = str(signal_label or "").strip().lower()
    if "bull" in label:
        return "background-color:#d9fbe6;color:#0f5132;border-radius:10px;padding:10px 12px;font-weight:700;"
    if "bear" in label:
        return "background-color:#ffe0e0;color:#8a1c1c;border-radius:10px;padding:10px 12px;font-weight:700;"
    if "beob" in label:
        return "background-color:#fff2d6;color:#8a5a00;border-radius:10px;padding:10px 12px;font-weight:700;"
    return "background-color:#eef1f4;color:#4b5563;border-radius:10px;padding:10px 12px;font-weight:700;"
...
def _fomo_v1525_class(label):
    lbl = str(label or "").lower()
    if "krit" in lbl:
        return "fomo-critical"
    if "erh" in lbl:
        return "fomo-elevated"
    if "beob" in lbl:
        return "fomo-watch"
    return "fomo-ok"


def _fomo_v1525_score_to_text(score, high_text, mid_text, low_text):
    try:
        score = float(score)
    except Exception:
        score = 0.0
...
def build_stock_fomo_package_v1525(result):
    ...
    high52 = _fomo_v1525_num(r.get("high52"), default=float("nan"))
    dist52 = _fomo_v1525_num(r.get("dist52"), default=float("nan"))
    if (not pd.notna(dist52)) and pd.notna(price) and pd.notna(high52) and high52 > 0:
        dist52 = price / high52 * 100.0

    ret21 = _fomo_v1525_num(r.get("ret21"), default=0)
    ret63 = _fomo_v1525_num(r.get("ret63"), default=0)
    stretch = _fomo_v1525_num(r.get("stretch_risk_score"), default=0)
    volume_quality = _fomo_v1525_num(r.get("volume_quality_score"), default=55)
    accumulation = _fomo_v1525_num(r.get("accumulation_score"), default=50)
    distribution = _fomo_v1525_num(r.get("distribution_pressure_score"), r.get("distribution_score"), default=20)
    breakout_volume = _fomo_v1525_num(r.get("breakout_volume_score"), default=50)
    leadership = _fomo_v1525_num(r.get("leadership_score"), default=50)
...
def build_market_fomo_package_v1525(market_info):
    ...
    high252 = _fomo_v1525_num(m.get("high252"), m.get("high52"), default=float("nan"))
    dist252 = _fomo_v1525_num(m.get("dist252"), default=float("nan"))
    if (not pd.notna(dist252)) and pd.notna(price) and pd.notna(high252) and high252 > 0:
        dist252 = price / high252 * 100.0
    ret21 = _fomo_v1525_num(m.get("ret21"), default=0)
    ret63 = _fomo_v1525_num(m.get("ret63"), default=0)
    ret126 = _fomo_v1525_num(m.get("ret126"), default=0)
...
def build_trigger_confluence_v1537(
    *,
    ...
    pattern_trigger = str(setup_pattern_pkg.get("trigger_label") or "")
    pattern_label = str(setup_pattern_pkg.get("label") or setup_pattern_pkg.get("phase") or "")
    if pattern_trigger or pattern_label:
        pattern_dir = _v1537_eval_label_text(
            pattern_trigger + " " + pattern_label,
            bullish_terms=["bestätigt", "bestaetigt", "erste", "pivot", "power", "flag"],
            bearish_terms=["bruch", "defensiv"],
            neutral_terms=["kein", "nicht", "beobachtung", "noch"],
        )
        if any(x in _v1537_norm_text(pattern_trigger) for x in ["kein", "noch nicht"]):
            pattern_dir = "neutral"
        components.append(_v1537_component("Spezialmuster", pattern_dir, f"{pattern_label or '-'} · Trigger: {pattern_trigger or '-'}.", 0.65))
...
def enrich_single_export_df_v1516(export_df, result, context=None):
    ...
    result_fields = {
        ...
        "Setup_Pattern_Label": ((result or {}).get("setup_pattern_pkg") or {}).get("label"),
        "Setup_Pattern_Phase": ((result or {}).get("setup_pattern_pkg") or {}).get("phase"),
        "Setup_Pattern_Typ": ((result or {}).get("setup_pattern_pkg") or {}).get("pattern_type"),
        "Setup_Pattern_Trigger": ((result or {}).get("setup_pattern_pkg") or {}).get("trigger_label"),
        ...
        "FOMO_Smart_Money_Handlung": ((result or {}).get("fomo_smart_money_pkg") or {}).get("action_hint"),
        "FOMO_Aktie_Label": ((result or {}).get("stock_fomo_pkg") or {}).get("label"),
...
def _radar_nested_num(result, pkg_key, value_key, default=0.0):
    try:
        pkg = result.get(pkg_key, {}) or {}
        return _radar_safe_num(pkg.get(value_key), default)
    except Exception:
        return default


def _radar_nested_text(result, pkg_key, value_key, default="-"):
    try:
        val = (result.get(pkg_key, {}) or {}).get(value_key, default)
        txt = str(val or default).strip()
        return txt if txt and txt.lower() not in {"nan", "none", "null"} else default
    except Exception:
        return default


def radar_chart_impulse_pack(result):
    """v17.2: eigener Radar-Schwerpunkt fuer charttechnisch interessante Impulse.

    Ziel: nicht die beste Firma suchen, sondern Kandidaten, bei denen Chartstruktur,
    Triggernaehe, Spezialmuster, Fibonacci/Strukturreaktion, MA10/20 und Volumen
    gerade einen konkreten technischen Impuls liefern koennten.
    """
    result = result or {}
    ...
    timing = _radar_safe_num(result.get("tb_score_100"), _radar_safe_num(result.get("tb_score"), 50))
    setup_conf = _radar_safe_num(result.get("setup_confidence"), 50)
    trend = _radar_safe_num(result.get("trend_quality_score"), 50)
    base = _radar_safe_num(result.get("base_quality_score"), 50)
    volume = _radar_safe_num(result.get("volume_quality_score"), 50)
    accum = _radar_safe_num(result.get("accumulation_score"), 0)
    distrib = _radar_safe_num(result.get("distribution_pressure_score"), 0)
    risk_bucket = radar_risk_bucket(result)
    fomo_label = str(((result.get("fomo_smart_money_pkg") or {}).get("label") or result.get("FOMO_Smart_Money_Label") or "") or "").lower()

    pattern_score = _radar_nested_num(result, "setup_pattern_pkg", "trigger_score", 0)
    pattern_label = _radar_nested_text(result, "setup_pattern_pkg", "label", "-")
    pattern_type = _radar_nested_text(result, "setup_pattern_pkg", "pattern_type", "-")
    fib_score = _radar_nested_num(result, "fibonacci_context_pkg", "confirmation_score", 0)
    fib_label = _radar_nested_text(result, "fibonacci_context_pkg", "confirmation_label", "-")
    wave_score = _radar_nested_num(result, "wave_structure_pkg", "confirmation_score", 0)
    wave_label = _radar_nested_text(result, "wave_structure_pkg", "confirmation_label", "-")
    confluence_score = _radar_nested_num(result, "trigger_confluence_pkg", "score", 0)
    confluence_label = _radar_nested_text(result, "trigger_confluence_pkg", "label", "-")
    ma10_label = str(result.get("ma10_timing_label") or result.get("MA10_Timing") or "").lower()
    ...
    if trigger in {"Aktiv", "Jetzt prüfbar"}:
        score += 18; reasons.append("Trigger direkt prüfbar")
    elif trigger in {"Nahe dran", "Fast prüfbar"}:
        score += 13; reasons.append("Trigger nahe dran")
    elif trigger in {"Frühe Beobachtung", "Früh interessant", "Beobachten", "Weiter beobachten"}:
        score += 7; reasons.append("früher technischer Aufbau")

    if valid:
        score += 10; reasons.append("valides Trade-Setup")
    if entry >= 70:
        score += 12; reasons.append("Einstiegsreife hoch")
    elif entry >= 58:
        score += 8; reasons.append("Einstiegsreife brauchbar")
    if timing >= 65:
        score += 10; reasons.append("TradingBoard konstruktiv")
    elif timing >= 55:
        score += 6
    if confluence_score >= 70:
        score += 11; reasons.append(f"Konfluenz {int(round(confluence_score))}/100")
    elif confluence_score >= 55:
        score += 7

    if pattern_score >= 55:
        score += 10; reasons.append(f"Spezialmuster: {pattern_type if pattern_type != '-' else pattern_label}")
    elif pattern_score >= 25:
        score += 5; reasons.append("Spezialmuster im Aufbau")
    if fib_score >= 55:
        score += 8; reasons.append("Fib-Reaktion bestätigt")
    elif fib_score >= 25:
        score += 4; reasons.append("Fib-Reaktion sichtbar")
    if wave_score >= 55:
        score += 8; reasons.append("Strukturtrigger bestätigt")
    elif wave_score >= 25:
        score += 4; reasons.append("Strukturreaktion sichtbar")

    if any(x in ma10_label for x in ["konstruktiv", "reclaim", "hält", "haelt"]):
        score += 7; reasons.append("MA10/20 konstruktiv")
    elif any(x in ma10_label for x in ["gedehnt", "überdehnt", "ueberdehnt"]):
        score -= 4; brakes.append("kurzfristig gedehnt")
    elif any(x in ma10_label for x in ["angeschlagen", "unter"]):
        score -= 6; brakes.append("unter kurzfristigem Trend")

    if volume >= 60 or accum > distrib + 8:
        score += 7; reasons.append("Volumen/Smart Money stützt")
    elif distrib > accum + 12:
        score -= 8; brakes.append("Distribution über Akkumulation")
    ...
    if risk_bucket == "hoch":
        score -= 16; brakes.append("Radar-Risiko hoch")
    elif risk_bucket == "erhöht":
        score -= 7; brakes.append("Radar-Risiko erhöht")
    if "kritisch" in fomo_label:
        score -= 12; brakes.append("FOMO kritisch")
    elif "erhöht" in fomo_label or "erhoeht" in fomo_label:
        score -= 5; brakes.append("FOMO erhöht")

    score = max(0.0, min(100.0, score))
    if score >= 75:
        label = "starker Chartimpuls"
    elif score >= 62:
        label = "charttechnisch sehr interessant"
    elif score >= 48:
        label = "technisch im Aufbau"
    elif score >= 35:
        label = "früh beobachten"
    else:
        label = "kein klarer Chartimpuls"

    trigger_text = " / ".join(reasons[:3]) if reasons else "kein aktiver technischer Trigger"
    brake_text = " / ".join(brakes[:2]) if brakes else "keine dominante technische Bremse"
    action = "Aktiv prüfen" if score >= 62 and risk_bucket != "hoch" else ("Watchlist eng verfolgen" if score >= 48 else "Nur beobachten")
    return {
        "score": round(score, 1),
        "label": label,
        ...
        "brake": brake_text,
        "action": action,
        "pattern": pattern_type if pattern_type != "-" else pattern_label,
        "confluence": confluence_label,
    }


def radar_chart_impulse_score(result):
    return float(radar_chart_impulse_pack(result).get("score", 0.0))


def radar_chart_impulse_reason(result):
    p = radar_chart_impulse_pack(result)
    return f"{p['label']} ({p['score']}/100). {p['trigger']}. Bremse: {p['brake']}."
...
def radar_professional_sort_score(row, result_map, style_name):
    ...
    if style_name == "Charttechnik":
        chart_pack = radar_chart_impulse_pack(r)
        chart_score = float(chart_pack.get("score", 0.0) or 0.0)
        score = chart_score * 1.15 + trigger_score * 8 + entry * 0.12 + tb * 0.10 + setup_conf * 0.08
        # Charttechnik darf fruehe Impulse finden, soll aber Hype/Risiko nicht blind hochziehen.
        if risk_bucket == "hoch":
            score -= 14
        elif risk_bucket == "erhöht":
            score -= 6
...
def leadership_label_phase_ui(score):
    try:
        s = float(score)
    except Exception:
        return "unauffällig"
...
def industry_strength_label_phase_ui(score):
    try:
        s = float(score)
    except Exception:
        return "gemischt"
...
def rs_accel_label_phase_ui(score):
    try:
        s = float(score)
    except Exception:
        return "neutral"
...
def build_setup_pattern_context_v162(chart_df=None, result=None):
    """Weicher Chart-Setup-Kontext fuer High Tight Pivot, Power Play und High Tight Flag.

    Bewusst nicht score-wirksam: Diese Muster sind selten und koennen bei zu
    harter Automatisierung Scheinsicherheit erzeugen. Die Funktion erkennt nur,
    ob eine Struktur in diese Richtung geht und ob ein Trigger bereits sichtbar
    ist.
    """
    result = result or {}
    out = {
        ...
        "phase": "Nicht aktiv",
        "pattern_type": "keines",
        "summary": "Kein belastbarer High-Tight-/Power-Play-Kontext erkennbar.",
        "action_hint": "Nicht aus diesem Baustein handeln; normale Entry-, S/R- und Bestätigungssignale nutzen.",
    ...
    try:
        ...
        if len(close) < 45 or len(high) < 45 or len(low) < 45:
            return out
        ...
        high40 = float(high.tail(40).max())
        low40 = float(low.tail(40).min())
        high60 = float(high.tail(min(60, len(high))).max())
        low60 = float(low.tail(min(60, len(low))).min())
        prev_high10 = float(high.iloc[:-1].tail(10).max()) if len(high) > 11 else high10
        prev_high20 = float(high.iloc[:-1].tail(20).max()) if len(high) > 21 else high20
        run20 = ((price / float(close.iloc[-21])) - 1) * 100 if len(close) > 21 and float(close.iloc[-21]) else np.nan
        run40 = ((price / float(close.iloc[-41])) - 1) * 100 if len(close) > 41 and float(close.iloc[-41]) else np.nan
        run60 = ((price / float(close.iloc[-61])) - 1) * 100 if len(close) > 61 and float(close.iloc[-61]) else np.nan
        ...
        # Candidate pattern conditions, deliberately soft.
        ...
        scores = {"High Tight Pivot": 0, "Power Play": 0, "High Tight Flag": 0}
        # High Tight Pivot: tight action near highs, close around/above short pivot.
        ...
        if above_short: scores["High Tight Pivot"] += 18; add_driver("Kurs hält MA10/MA20")
        if near_pivot or breakout: scores["High Tight Pivot"] += 24; add_driver(f"Pivotbereich um {_v1533_1_fmt_price(pivot_zone)}")
        if vol_conf: scores["High Tight Pivot"] += 10; add_driver(f"Volumen über Schnitt ({vol_ratio:.2f}x)")

        # Power Play: strong fast move, high/tight hold, little pullback.
        ...
        # High Tight Flag: large move from recent low, then shallow/tight flag near top.
        ...
        if best_score < 45:
            ...
            phase = "Nicht aktiv"
            pattern_type = "keines"
            summary = "Kein belastbarer High-Tight-/Power-Play-Kontext. Normale Chart- und Entry-Logik bleibt führend."
            action = "Nicht aus diesem Baustein handeln; S/R, Entry-Zone, Candlestick und Konfluenz prüfen."
        elif best_type == "High Tight Pivot":
            label = "möglicher High Tight Pivot"
            phase = "Enger Pivot nahe Hoch"
            pattern_type = best_type
            summary = "Kurs handelt eng nahe dem Hoch. Das kann ein kurzer Pivot sein, braucht aber einen klaren Ausbruch oder eine bestätigte Reaktion."
            action = f"Aktiv prüfen erst bei Ausbruch/Stabilisierung über dem Pivotbereich um {_v1533_1_fmt_price(pivot_zone)}; defensiver, wenn MA10/MA20 brechen."
        elif best_type == "Power Play":
            label = "mögliches Power Play"
            phase = "Starker kurzer Impuls"
            pattern_type = best_type
            summary = "Sehr dynamischer kurzfristiger Move. Stärke ist vorhanden, aber FOMO-/Überdehnungsrisiko beachten."
            action = f"Nicht hinterherlaufen. Einstieg nur bei enger Konsolidierung, Reclaim oder Pullback in Richtung MA10/MA20 ({_v1533_1_fmt_price(ma10)} / {_v1533_1_fmt_price(ma20)})."
        else:
            label = "mögliche High Tight Flag"
            phase = "Starker Move mit enger Flagge"
            pattern_type = best_type
            summary = "Nach starkem Anstieg hält der Kurs relativ eng im oberen Bereich. Das kann konstruktiv sein, ist aber selten und braucht Ausbruchsbestätigung."
            action = f"Trigger wäre ein sauberer Ausbruch über den Flag-/Pivotbereich um {_v1533_1_fmt_price(pivot_zone)} mit Bestätigung; Bruch unter MA20/Flag-Tief macht es defensiver."

        # Trigger state.
        trigger_score = 0
        tdrivers = []
        if breakout:
            trigger_score += 35; tdrivers.append(f"Kurs über Pivot {_v1533_1_fmt_price(pivot_zone)}")
        elif near_pivot:
            trigger_score += 18; tdrivers.append(f"Kurs nahe Pivot {_v1533_1_fmt_price(pivot_zone)}")
        if above_short:
            trigger_score += 18; tdrivers.append("MA10/MA20 halten")
        if tight_recent:
            trigger_score += 14; tdrivers.append("kurzfristig enge Struktur")
        if vol_conf:
            trigger_score += 18; tdrivers.append(f"Volumenbestätigung {vol_ratio:.2f}x")
        if near_high:
            trigger_score += 10; tdrivers.append("nahe Hochbereich")
        trigger_score = int(round(min(trigger_score, 100))) if best_score >= 45 else 0
        if best_score < 45:
            trig_label = "kein Trigger"
            trig_text = "Kein aktiver Spezialmuster-Trigger."
            trig_action = "Normale Analysebausteine verwenden."
        elif trigger_score >= 72:
            trig_label = "bestätigt"
            trig_text = "Spezialmuster-Trigger wirkt bestätigt: Pivot/Struktur, kurzfristiger Trend und Bestätigung ziehen zusammen."
            trig_action = "Nur im Zusammenspiel mit Aktion, Risiko, Entry-Zone und Konfluenz als aktiven Trigger werten."
        elif trigger_score >= 52:
            trig_label = "erste Bestätigung"
            trig_text = "Erste Bestätigung sichtbar, aber noch nicht vollständig."
            trig_action = "Vorbereiten/aktiv prüfen; auf Schlusskurs, Volumen und Folgetag achten."
        elif trigger_score >= 30:
            trig_label = "unter Beobachtung"
            trig_text = "Struktur ist interessant, aber Trigger ist noch unvollständig."
            trig_action = "Watchlist/Alarm am Pivotbereich setzen; nicht blind handeln."
        else:
            trig_label = "noch nicht erreicht"
        ...
        out.update({
            ...
            "summary": summary,
            "action_hint": action,
            "active_zone_text": f"Pivot/Triggerbereich um {_v1533_1_fmt_price(pivot_zone)}" if pivot_zone else "n/a",
            ...
            "metrics": {
                ...
                "Pullback_20T_Hoch_%": round(float(pullback20), 1) if pd.notna(pullback20) else "n/a",
                "Range_10T_%": round(float(tight10), 1) if pd.notna(tight10) else "n/a",
...
def compact_action_text_phase_ui(action_label):
    ...
    mapping = {
        ...
        "stop enger": "Warnsignale nehmen zu, Risikokontrolle wird wichtiger.",
        "exit prüfen": "Die Lage kippt, Schutz des Kapitals steht im Vordergrund.",
    }
    return mapping.get(a, "Die Aktion folgt Timing, Risiko und Triggerstatus.")
...
def evaluate_market_filter(benchmark_df):
    ...
    high252 = safe_last(close.rolling(252).max(), float(close.max()) if len(close.dropna()) else np.nan)
    low252 = safe_last(close.rolling(252).min(), float(close.min()) if len(close.dropna()) else np.nan)
...
if result is not None:
    ...
    .compact-summary-card.is-primary{
        border:1px solid rgba(96,165,250,0.28);
        border-left:5px solid rgba(96,165,250,0.90);
        border-radius:16px;
        padding:12px 12px;
    ...
    .compact-summary-card.is-primary .compact-summary-sub{font-size:0.72rem;color:#cbd5e1;margin-top:8px;line-height:1.28;}
    .overview-primary-grid{margin:10px 0 14px 0;}
    .overview-primary-card{
        border:1px solid rgba(96,165,250,0.30);
        border-left:6px solid rgba(96,165,250,0.92);
        border-radius:22px;
        padding:18px 18px 16px 18px;
        min-height:164px;
        background:linear-gradient(180deg, rgba(30,41,59,0.88), rgba(15,23,42,0.66));
        box-shadow:0 18px 38px rgba(2,6,23,0.24);
        display:flex;
        flex-direction:column;
        justify-content:flex-start;
    }
    .overview-primary-title{font-size:0.76rem;color:#bfdbfe;text-transform:uppercase;letter-spacing:0.055em;font-weight:950;line-height:1.1;}
    .overview-primary-value{font-size:clamp(1.32rem,1.8vw,1.72rem);font-weight:950;color:#f8fafc;line-height:1.08;margin-top:10px;word-break:break-word;overflow-wrap:anywhere;}
    .overview-primary-sub{font-size:0.86rem;color:#cbd5e1;line-height:1.38;margin-top:10px;}
    .overview-context-grid{margin:4px 0 16px 0;}
    ...
    .quiet-section-card .premium-value{font-size:0.98rem!important;font-weight:760!important;color:#dbeafe!important;}
    .quiet-section-card .premium-sub{font-size:0.80rem!important;color:#94a3b8!important;}
    ...
    with st.expander("Technische Chartdetails", expanded=False):
        if chart_structures:
            ...
            st.markdown(
                f"""
                <div class="section-card" style="padding:0.85rem 0.95rem;margin:0.55rem 0 0.85rem 0;">
                    <div class="premium-title">{html.escape(str(setup_pattern_pkg.get('phase', 'Nicht aktiv')))}</div>
                    <div class="premium-value" style="font-size:1.02rem;">{html.escape(str(setup_pattern_pkg.get('label', 'n/a')).capitalize())}</div>
    ...
    st.markdown(
        f"""<div class="candle-dash-grid">
            <div class="candle-dash-card">
                <h4>Marktumfeld</h4>
                <div class="candle-dash-main">{regime_ctx.get('label','Neutral')}</div>
                <div class="candle-dash-sub">{regime_ctx.get('summary','-')}</div>
            </div>
            <div class="candle-dash-card">
                <h4>Führung</h4>
                <div class="candle-dash-main">{regime_ctx.get('leadership','Keine klare Führung')}</div>
                <div class="candle-dash-sub">Zeigt, wer das Umfeld aktuell eher trägt.</div>
            </div>
            <div class="candle-dash-card">
                <h4>Einordnung</h4>
                <div class="candle-dash-main">Kontext für Signale</div>
                <div class="candle-dash-sub">{final_timing_reason if 'final_timing_reason' in locals() else regime_ctx.get('summary','-')}</div>
            </div>
        </div>""",
        unsafe_allow_html=True,
    )
    ...
    stock_fomo_pkg_ui = result.get("stock_fomo_pkg") or build_stock_fomo_package_v1525(result)
    ...
    for _col, (_title, _value, _sub) in zip(primary_cols, primary_summary_data):
        with _col:
            st.markdown(
                f"""
                <div class="overview-primary-card">
                    <div class="overview-primary-title">{_title}</div>
                    <div class="overview-primary-value">{_value}</div>
                    <div class="overview-primary-sub">{_sub}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
    ...
    with st.expander("FOMO / Smart Money", expanded=False):
        ...
        for _col, _title, _pkg in [(_fomo_cols[0], "Einzelaktie", stock_fomo_pkg_ui), (_fomo_cols[1], "Gesamtmarkt", market_fomo_pkg_ui)]:
            with _col:
                _lbl = str((_pkg or {}).get("label", "unauffällig")).capitalize()
                _summary = html.escape(str((_pkg or {}).get("summary", "-")))
                _action = html.escape(str((_pkg or {}).get("action_hint", "")))
                _reasons = (_pkg or {}).get("reasons", []) or []
                st.markdown(f"**{_title}: {_lbl}**")
                st.caption(_summary)
                if _action:
    ...
    if is_pro_mode or is_expert_mode:
        ...
        with t1:
            ...
            tech_df = pd.DataFrame({
                ...
                "Wert": [
                    f"{price:.2f}", f"{ma10:.2f}", f"{ma20:.2f}", f"{ma50:.2f}", f"{ma150:.2f}", f"{ma200:.2f}",
